...
# List - is used to create a relationship then will return a list of objects
...
# generate_password_hash - allows us to hash passwords for security purposes



app = Flask(__name__) # creating an instance of the Flask class for our app to use

app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+mysqlconnector://root:Groovin@localhost/ecom'
...
class Customer(Base):
    ...
    user_name: Mapped[str] = mapped_column(db.String(150), nullable= False, unique= True)
    password: Mapped[str] = mapped_column(db.String(500))
...
def add_product():
    ...
    new_product = Products(product_name= product_data['product_name'], 
...
# Get a single product by id with a GET request
...
def add_order():
    ...
    new_order = Orders(order_date= date.today(), 
